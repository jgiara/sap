...
class Users{
	...
	public function email_exists($Email) {
		...
		$query = $this->db->prepare("SELECT COUNT(eagle_id) FROM Users WHERE email= ? and status != 'Graduated'");
...
public function register( $Password, $Email, $First_Name, $Last_Name, $Eagle_Id, $Address, $Phone, $Type){
	...
	public function login($Email, $Password) {
		...
		$query = $this->db->prepare("SELECT password FROM Users WHERE email = ? and status != 'Graduated'");
		$query->bindValue(1, $Email);
	...
	public function updateLastLogin($Email) {
		...
		$query = $this->db->prepare("UPDATE Users set last_login = now() where email = ?");
		$query->bindValue(1, $Email);
